      ...
      @for (feat of features; track feat.title) {
        <div class="glass-card flex flex-col gap-3 p-5">
          <div
            ...
            [style.box-shadow]="'0 0 14px ' + feat.color + '35'"
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" [attr.stroke]="feat.color" class="size-5">
              <path stroke-linecap="round" stroke-linejoin="round" [attr.d]="feat.icon" />
            </svg>
          </div>
          <div>
            <p class="text-sm font-semibold text-white">{{ feat.title }}</p>
            <p class="mt-0.5 text-xs leading-relaxed text-gray-500">{{ feat.desc }}</p>
          </div>
        </div>
      }
    </div>
  ...
  features: Feature[] = [
    {
      ...
      color: '#a78bfa',
      title: 'Auto-dismiss',
      desc: 'Toasts vanish after 4s with a glowing progress bar.',
    },
    {
    ...
    },
    {
      ...
      color: '#fbbf24',
      title: 'Delay appearance',
      desc: 'Show toasts after a short delay for smooth UX.',
    },
  ];
}